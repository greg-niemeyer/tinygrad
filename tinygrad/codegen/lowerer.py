    ...
    if x.op in ReduceOps:
      ...
      if x.op is ReduceOps.WMMA:
        ...
        ret = UOp(UOps.WMMA, dtype=dtype.vec(wmma_sz[2]), src=(
          ...
          UOp(UOps.CONST, dtype.vec(wmma_sz[2]), arg=0.0)), arg=x.arg)
        expand = UOp(UOps.EXPAND, dtype, tuple(UOp(UOps.GEP, dtype, (ret,), i) for i in range(wmma_sz[2])), arg=upcast_axes[2])
        return expand
