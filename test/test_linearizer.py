...
class TestLinearizer(unittest.TestCase):
  ...
  def test_hand_coded_unsplit_kernel(self):
    ...
    r = UOp(UOps.REDUCE_AXIS, dtypes.float, (load,), (BinaryOps.ADD, (0, 1)))
  ...
  def test_hand_coded_split_kernel(self):
    ...
    r_1 = UOp(UOps.REDUCE_AXIS, dtypes.float, (load_1,), (BinaryOps.ADD, (1,)))
    ...
    r_2 = UOp(UOps.REDUCE_AXIS, dtypes.float, (load_2,), (BinaryOps.ADD, (2,)))
  ...
  def test_hand_coded_nested_kernel(self):
    ...
    inbuf_1 = a.lazydata.buffer
    # NOTE: we want outbuf_1 generated by the lower_schedule_item
    #outbuf_1 = Buffer(a.device, store_1.st_arg.size, g0.dtype).allocate()
    #f = CompiledRunner(kernel_1.to_program())
    #f.exec([outbuf_1, inbuf_1])
    #inbuf_2 = outbuf_1
    ...
    #f.exec([outbuf_2, inbuf_2])
    #print(np.frombuffer(outbuf_2.as_buffer(), _to_np_dtype(dtypes.float)))
    #ScheduleItem(lsi.ast, tuple(x.buffer for x in lsi.outputs+lsi.inputs if x.size != 0), lsi.metadata)
    from tinygrad.engine.schedule import ScheduleItem
    from tinygrad.engine.realize import lower_schedule_item
    # NOTE: we pass sink_2 directly for now because we haven't ported the split logic to Kernel yet
    si = ScheduleItem(sink_2, [outbuf_2, inbuf_1])
    eil = lower_schedule_item(si)
